...
### v1.4.3
- **Weekday patterns are no longer invisible** — the weekend-vs-weekday section returned nothing at all when no effect reached significance, so the feature appeared to be missing entirely. It now always shows its header, and explains what it needs (Welch t-test at p < 0.05, at least 4 logged days per group) when there is nothing to report
